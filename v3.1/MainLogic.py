...
class LogicMixin(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # self.upload_dir = "Assets/history"
        ...
        try:
            ...
            if input_type == "图片":
                ...
                if file and os.path.exists(file):
                    # self._copy_file_to_uploads(file)
    ...
    def _copy_file_to_uploads(self, source_path: str):
        if not source_path:
            return
        try:
            ...
            self.statusbar.showMessage(f"文件已复制到: {destination_path}", 5000)  # Message disappears after 5s

        except Exception as e:
            QMessageBox.critical(self, "复制文件失败", f"无法将文件复制到 '{self.upload_dir}':\n{str(e)}")
